      ...
      <div className="relative z-10 flex min-h-screen">
        ...
        <main className="min-w-0 flex-1 overflow-x-hidden">
          ...
          <div className="mx-auto w-full max-w-[1360px] px-3 py-5 sm:px-5 lg:px-6">
            ...
            {activeView === "Workflows" && <WorkflowsPage workflows={filteredWorkflows} executions={executions} loading={loading} />}
            {activeView === "Executions" && <ExecutionsPage executions={filteredExecutions} workflows={workflows} loading={loading} />}
            {activeView === "Problems" && <ProblemsPage executions={executions} workflows={workflows} loading={loading} />}
  ...
  return (
    <div className="sticky top-0 z-30 border-b border-slate-200/70 bg-white/75 px-4 py-3 backdrop-blur-2xl dark:border-white/10 dark:bg-[#070B14]/75 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-[1500px] items-center gap-3">
        ...
        <div className="hidden text-xs text-slate-500 dark:text-slate-400 xl:block">Last sync {lastSync ? formatTime(lastSync) : "-"}</div>
      </div>
    </div>
  );
}
...
function WorkflowsPage({ workflows, executions, loading }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <WorkflowTable workflows={workflows} executions={executions} loading={loading} />
    </div>
  );
}

function ExecutionsPage({ executions, workflows, loading }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <ExecutionsTable executions={executions} workflows={workflows} loading={loading} />
    </div>
  );
}
...
function SummaryCard({ title, value, icon: Icon, tone = "cyan" }) {
  ...
  return (
    <motion.div whileHover={{ y: -3 }} className="rounded-[24px] border border-slate-200/70 bg-white/75 p-5 shadow-premium backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.045]">
...
function ExecutionsList({ executions, workflows, loading }) {
  return (
    <Panel title="Recent Executions" subtitle="Latest loaded n8n runs" icon={Activity}>
      <div className="space-y-2">
        {loading && <SkeletonRows small />}
        {!loading && executions.map((execution) => <ExecutionRow key={execution.id} execution={execution} workflows={workflows} compact />)}
      </div>
    </Panel>
  );
}

function ExecutionsTable({ executions, workflows, loading, problemOnly = false }) {
  return (
    <Panel title={problemOnly ? "Failed Executions" : "All Executions"} subtitle="Running, completed, and failed runs" icon={Activity}>
      <div className="space-y-2">
        ...
        {!loading && executions.map((execution) => <ExecutionRow key={execution.id} execution={execution} workflows={workflows} />)}
      </div>
    </Panel>
  );
}
